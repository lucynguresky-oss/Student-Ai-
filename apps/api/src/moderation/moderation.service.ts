...
// auto-remove on a shaky signal â€” borderline cases go to human review.
...
export class ModerationService {
   ...
   * and warns the author if the post is removed. Never throws to the caller â€”
  ...
  async screenPost(postId: string): Promise<ModerationStatus> {
    ...
    if (!apiKey) {
      ...
      this.logger.warn(
        `No ANTHROPIC_API_KEY set â€” skipping AI moderation for post ${postId} (auto-approved).`,
      );
    ...
    try {
      verdict = await this.classify(post.caption ?? '', {
        mediaTypes: post.media.map((m: any) => m.type),
        subject: post.subject?.nameEn ?? null,
      });
    } catch (err) {
      this.logger.error(
  ...
  private async warnAuthor(
    ...
    this.logger.log(`Warned ${authorId} â€” post ${postId} removed: ${reason}`);
  }
}
...
export function decideAction(v: ModerationVerdict): ModerationStatus {
  ...
  return ModerationStatus.FLAGGED; // borderline â†’ human review
}
